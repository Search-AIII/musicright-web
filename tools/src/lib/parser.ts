...
function detectStatus(text: string, platformIndex: number): AccountStatus {
  // lookback 60 chars (catches "no ASCAP", "don't have BMI")
  // lookahead 20 chars only — wider window lets "No MLC" taint nearby platforms
  const ctx = text.slice(Math.max(0, platformIndex - 60), platformIndex + 20).toLowerCase();
